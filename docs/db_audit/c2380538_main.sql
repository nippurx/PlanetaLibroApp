...
--
-- Servidor: localhost
-- Tiempo de generación: 22-07-2026 a las 18:04:49
...
CREATE TABLE `ebooks_categories` (
  ...
  `ebooks_categories_description` longtext,
...
--
-- Estructura de tabla para la tabla `ebook_regeneration_queue`
--

CREATE TABLE `ebook_regeneration_queue` (
  `regeneration_id` bigint(20) UNSIGNED NOT NULL,
  `ebooks_books_id` int(11) NOT NULL,
  `reason` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'legacy_compatibility_manifest',
  `status` enum('pending','processing','failed','completed','ignored') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'pending',
  `target_generator` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'epub2html2',
  ...
  `compatibility_manifest_generated_at` datetime DEFAULT NULL,
...
--
-- Estructura de tabla para la tabla `user_book_annotations`
--
...
CREATE TABLE `user_book_annotations` (
  ...
  `ebooks_books_id` int(11) NOT NULL,
  `annotation_type` enum('highlight','note','bookmark') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'highlight',
  `start_fragment` mediumint(8) UNSIGNED NOT NULL,
  `start_offset` int(10) UNSIGNED NOT NULL,
  `end_fragment` mediumint(8) UNSIGNED NOT NULL,
  `end_offset` int(10) UNSIGNED NOT NULL,
  `exact_text` text COLLATE utf8mb4_unicode_ci NOT NULL,
  `prefix_text` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `suffix_text` varchar(512) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `content_version` varchar(128) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  `anchor_version` smallint(5) UNSIGNED NOT NULL DEFAULT '1',
  `note_text` text COLLATE utf8mb4_unicode_ci,
  `color_code` tinyint(3) UNSIGNED NOT NULL DEFAULT '1',
  `client_request_id` char(36) CHARACTER SET ascii COLLATE ascii_bin DEFAULT NULL,
  `revision` int(10) UNSIGNED NOT NULL DEFAULT '1',
  `created_at` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
  `updated_at` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Destacados y notas privadas ancladas al contenido de libros';

-- --------------------------------------------------------

--
-- Estructura de tabla para la tabla `user_book_entitlements`
--

CREATE TABLE `user_book_entitlements` (
  `id` bigint(20) UNSIGNED NOT NULL,
  `user_id` int(20) NOT NULL,
  `book_id` int(11) NOT NULL,
  `status` enum('active','revoked','expired') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'active',
  `source_type` enum('manual_grant','gift','purchase','promotion','license') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'manual_grant',
  `starts_at` datetime NOT NULL,
  `expires_at` datetime DEFAULT NULL,
  `granted_at` datetime NOT NULL,
  `granted_by_user_id` int(20) DEFAULT NULL,
...
--
-- Indices de la tabla `ebook_regeneration_queue`
--
ALTER TABLE `ebook_regeneration_queue`
  ADD PRIMARY KEY (`regeneration_id`),
  ADD UNIQUE KEY `uq_regeneration_book_generator` (`ebooks_books_id`,`target_generator`),
  ADD KEY `idx_regeneration_status_detected` (`status`,`detected_at`),
  ADD KEY `idx_regeneration_book` (`ebooks_books_id`),
  ADD KEY `idx_regeneration_processing_lock` (`status`,`locked_at`);
...
--
-- Indices de la tabla `user_book_annotations`
--
ALTER TABLE `user_book_annotations`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `uq_uba_user_request` (`user_id`,`client_request_id`),
  ADD KEY `idx_uba_user_book_location` (`user_id`,`ebooks_books_id`,`start_fragment`,`start_offset`,`id`),
  ADD KEY `idx_uba_user_updated` (`user_id`,`updated_at`,`id`),
  ADD KEY `idx_uba_book` (`ebooks_books_id`,`id`),
  ADD KEY `idx_uba_user_book_type_location` (`user_id`,`ebooks_books_id`,`annotation_type`,`start_fragment`,`start_offset`,`id`);

--
-- Indices de la tabla `user_book_entitlements`
--
ALTER TABLE `user_book_entitlements`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_entitlement_authorization` (`user_id`,`book_id`,`status`,`starts_at`,`expires_at`),
  ADD KEY `idx_entitlement_user_library` (`user_id`,`status`,`expires_at`),
  ADD KEY `idx_entitlement_book` (`book_id`),
  ADD KEY `idx_entitlement_granted_by` (`granted_by_user_id`);
...
--
-- AUTO_INCREMENT de la tabla `ebook_regeneration_queue`
--
ALTER TABLE `ebook_regeneration_queue`
  MODIFY `regeneration_id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT;
...
--
-- AUTO_INCREMENT de la tabla `user_book_annotations`
--
ALTER TABLE `user_book_annotations`
  MODIFY `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT de la tabla `user_book_entitlements`
--
ALTER TABLE `user_book_entitlements`
  MODIFY `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT;
...
--
-- Filtros para la tabla `user_book_annotations`
--
ALTER TABLE `user_book_annotations`
  ADD CONSTRAINT `fk_uba_user` FOREIGN KEY (`user_id`) REFERENCES `user_table` (`userid`) ON DELETE CASCADE;
